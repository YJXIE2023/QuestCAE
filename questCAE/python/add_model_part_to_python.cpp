...
#include "python/add_model_part_to_python.hpp"
...
namespace Quest{
    ...
    ModelPart::MeshType::Pointer ModelPartGetMesh2(ModelPart& rModelPart, ModelPart::IndexType MeshIndex){
        ...
        for(ModelPart::IndexType i = number_of_meshes ; i < MeshIndex + 1 ; i++)
            rModelPart.GetMeshes().push_back(Quest::make_shared<ModelPart::MeshType>(empty_mesh.Clone()));
    ...
    {
        if (!QuestComponents<Element>::Has(ElementName)) {
            std::stringstream msg;
            QuestComponents<Element> instance; 
            instance.PrintData(msg);

            QUEST_ERROR << "The Element \"" << ElementName << "\" is not registered!\nMaybe you need to import the application where it is defined?\nThe following Elements are registered:\n" << msg.str() << std::endl;
        }
    ...
    {
        if (!QuestComponents<Condition>::Has(ConditionName)) {
            std::stringstream msg;
            QuestComponents<Condition> instance; 
            instance.PrintData(msg);

            QUEST_ERROR << "The Condition \"" << ConditionName << "\" is not registered!\nMaybe you need to import the application where it is defined?\nThe following Conditions are registered:\n" << msg.str() << std::endl;
        }
    ...
    ModelPart::MasterSlaveConstraintType::Pointer CreateNewMasterSlaveConstraint1(
        ModelPart& rModelPart,
        ...
        if (!QuestComponents<MasterSlaveConstraint>::Has(ConstraintName)) {
            std::stringstream msg;
            QuestComponents<MasterSlaveConstraint> instance; 
            instance.PrintData(msg);

            QUEST_ERROR << "The Constraint \"" << ConstraintName << "\" is not registered!\nMaybe you need to import the application where it is defined?\nThe following Constraints are registered:\n" << msg.str() << std::endl;
        }
    ...
    const std::unordered_set<std::string> GetNonHistoricalVariablesNames(ModelPart& rModelPart, TContainerType& rContainer, bool doFullSearch=false) {
        ...
        if(doFullSearch) {
            if(rContainer.size() == 0) {
                QUEST_WARNING("DEBUG") << "Checking and empty container" << std::endl;
            } else {
    ...
    void AddModelPartToPython(pybind11::module& m){
        ...
        py::class_<ModelPart, Quest::shared_ptr<ModelPart>, DataValueContainer, Flags >(m,"ModelPart")
        ...
        .def_property("Geometries", [](ModelPart& self) { return self.Geometries(); },
            [](ModelPart& self, ModelPart::GeometriesMapType& geometries) {
                QUEST_ERROR << "Setting geometries is not allowed! Trying to set value of ModelPart::Geometries."; })
        ...
        .def("AddNodalSolutionStepVariable", AddNodalSolutionStepVariable<Array1d<double, 3 > >)
        ...
        .def("HasNodalSolutionStepVariable", HasNodalSolutionStepVariable<Array1d<double, 3 > >)
        ...
        .def_property("SubModelParts",  [](ModelPart& self){ return self.SubModelParts(); },
                                        [](ModelPart& self, ModelPart::SubModelPartsContainerType& subs){ QUEST_ERROR << "setting submodelparts is not allowed"; })
