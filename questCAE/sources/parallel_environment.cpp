...
namespace Quest{
    ...
    ParallelEnvironment::ParallelEnvironment()
    {
        ...
        RegisterCommunicatorFactoryDetail<const std::string>([](ModelPart& rModelPart, const std::string& rDataCommunicatorName)->Communicator::UniquePointer{
            ...
            return Quest::make_unique<Communicator>(r_data_communicator);
        });
        RegisterCommunicatorFactoryDetail<const DataCommunicator>([](ModelPart& rModelPart, const DataCommunicator& rDataCommunicator)->Communicator::UniquePointer{
            QUEST_ERROR_IF(rDataCommunicator.IsDistributed()) << "Trying to create an serial communicator with a distributed data communicator." << std::endl;
            return Quest::make_unique<Communicator>(rDataCommunicator);
        });
        ...
        RegisterFillCommunicatorFactoryDetail<const std::string>([](ModelPart& rModelPart, const std::string& rDataCommunicatorName)->FillCommunicator::Pointer{
            const auto& r_data_communicator = ParallelEnvironment::GetDataCommunicator("Serial");
            return Quest::make_shared<FillCommunicator>(rModelPart, r_data_communicator);
        });
        RegisterFillCommunicatorFactoryDetail<const DataCommunicator>([](ModelPart& rModelPart, const DataCommunicator& rDataCommunicator)->FillCommunicator::Pointer{
            QUEST_ERROR_IF(rDataCommunicator.IsDistributed()) << "Trying to create an serial communicator with a distributed data communicator." << std::endl;
            return Quest::make_shared<FillCommunicator>(rModelPart, rDataCommunicator);
        });
    }
    ...
    std::string ParallelEnvironment::RetrieveRegisteredName(const DataCommunicator& rComm)
    {
        ...
        QUEST_ERROR << "the required communicator was not registered" << std::endl;
    }
    ...
    ParallelEnvironment& ParallelEnvironment::GetInstance()
    {
        if (!mpInstance) {
            ...
            if (!mpInstance) {
                QUEST_ERROR_IF(mDestroyed) << "Accessing ParallelEnvironment after its destruction" << std::endl;
                Create();
            }
        }
    ...
    {
        QUEST_ERROR_IF(MPIIsInitialized() || MPIIsFinalized())
        ...
        const bool Default)
    {
        ...
        if (found == mDataCommunicators.end())
        {
            ...
            QuestComponents<DataCommunicator>::Add(Name, *(pair_iterator->second));
            ...
            QUEST_WARNING("ParallelEnvironment")
    ...
    {
        QUEST_ERROR_IF(Name == mDefaultCommunicator->first)
        ...
        QUEST_WARNING_IF("ParallelEnvironment", num_erased == 0)
        ...
        if (num_erased == 1)
        {
            QuestComponents<DataCommunicator>::Remove(Name);
        }
    }
    ...
    DataCommunicator& ParallelEnvironment::GetDataCommunicatorDetail(const std::string& rName) const
    {
        ...
        QUEST_ERROR_IF(found == mDataCommunicators.end())
