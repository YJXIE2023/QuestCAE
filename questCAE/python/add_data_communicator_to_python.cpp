...
#include "python/add_data_communicator_to_python.hpp"
...
namespace Quest::Python{
    ...
    std::vector<TValue> VectorBroadcastWarpper(
    ...
    template<typename TModelType, typename TDataType>
    void AddDataCommunicatorMethodForDataType(TModuleType& rDataCommunicatorModule){
        std::string arg_text;
        if constexpr(std::is_same_v<TDataType, int>){
            arg_text = "Int";
        } else if constexpr(std::is_same_v<TDataType, long>){
            arg_text = "Double";
        } else if constexpr(std::is_same_v<TDataType, Array1d<double, 3>>){
            arg_text = "Array3";
        } else if constexpr(std::is_same_v<TDataType, Array1d<double, 4>>){
            arg_text = "Array4";
        } else if constexpr(std::is_same_v<TDataType, Array1d<double, 6>>){
            arg_text = "Array6";
        } else if constexpr(std::is_same_v<TDataType, Array1d<double, 9>>){
            arg_text = "Array9";
    ...
    void AddDataCommunicatorToPython(pybind11::module &m){
        ...
        AddDataCommunicatorMethodForDataType<data_communicator_module_type, Array1d<double, 3>>(data_communicator_module);
        AddDataCommunicatorMethodForDataType<data_communicator_module_type, Array1d<double, 4>>(data_communicator_module);
        AddDataCommunicatorMethodForDataType<data_communicator_module_type, Array1d<double, 6>>(data_communicator_module);
        AddDataCommunicatorMethodForDataType<data_communicator_module_type, Array1d<double, 9>>(data_communicator_module);
        ...
        data_communicator_module.def("Barrier", &DataCommunicator::Barrier)
            ...
            .def("GetDefault", []() -> DataCommunicator& {
                QUEST_WARNING("DataCommunicator") << "This function is deprecated, please retrieve the DataCommunicator through the ModelPart (or by name in special cases)" << std::endl;
