...
#include "python/add_properties_to_python.hpp"
#include "python/containers_interface.hpp"


using AccessorBindType = std::unique_ptr<Quest::Accessor>;
...
namespace Quest::Python{
    ...
    void SetArrayValue(
        ...
        if(rData.size() != 3){
            QUEST_ERROR << "attempting to construct an array<double,3> by passing a list with wrong size. Input size is " << rData.size() << std::endl;
        }

        Array1d<double,3> tmp;
    ...
    template< class TContainerType, class TVariableType >
    typename TVariableType::Type GetValueHelperFunction1( 
    ...
    template< class TContainerType, class XVariableType, class YVariableType>
    typename Properties::TableType& GetTableHelperFunction1( 
        TContainerType& rContainer,
        const XVariableType& XVar,
        const YVariableType& YVar 
    ){
    ...
    void AddInterfaceToAccessorFold(pybind11::class_<Properties, Properties::Pointer, Properties::BaseType>& pyProperties) {
        ...
        .def("GetAccessor", [](Properties &rProperties, Variable<TVariableType> &rVariable) { 
                ...
                QUEST_ERROR_IF(!*accessor) << "Trying to get a consumed or invalid Accessor." << std::endl;
                    
                return accessor; 
            }, py::return_value_policy::reference_internal)
        .def("SetAccessor", [](Properties &rProperties, Variable<TVariableType> &rVariable, std::unique_ptr<Accessor>& rpAccessor) {
                QUEST_ERROR_IF(!rpAccessor) << "Trying to set a consumed or invalid Accessor. Accessors are unique. Please create a different one." << std::endl;
    ...
    void AddPropertiesToPython(pybind11::module& m){
        auto properties_module = py::class_<Properties, Properties::Pointer, Properties::BaseType>(m, "Properties")
            .def(py::init<Quest::Properties::IndexType>())
            .def(py::init<const Properties&>())
            .def("__setitem__", SetValueHelperFunction1<Properties, Variable<Array1d<double, 6>>>)
            .def("__getitem__", GetValueHelperFunction1<Properties, Variable<Array1d<double, 6>>>)
            .def("Has", PropertiesHasHelperFunction<Properties, Variable<Array1d<double, 6>>>)
            .def("SetValue", SetValueHelperFunction1<Properties, Variable<Array1d<double, 6>>>)
            .def("GetValue", GetValueHelperFunction1<Properties, Variable<Array1d<double, 6>>>)
            .def("Erase", EraseHelperFunction1<Properties, Variable<Array1d<double, 6>>>)

            .def("__setitem__", SetValueHelperFunction1<Properties, Variable<Array1d<double, 3>>>)
            .def("__getitem__", GetValueHelperFunction1<Properties, Variable<Array1d<double, 3>>>)
            .def("Has", PropertiesHasHelperFunction<Properties, Variable<Array1d<double, 3>>>)
            .def("SetValue", SetValueHelperFunction1<Properties, Variable<Array1d<double, 3>>>)
            .def("GetValue", GetValueHelperFunction1<Properties, Variable<Array1d<double, 3>>>)
            .def("Erase", EraseHelperFunction1<Properties, Variable<Array1d<double, 3>>>)
            ...
            .def("GetValue", [](Properties &rProperties, const Variable<Matrix> &rVariable, const GeometryType &rGeometry, const Vector &rShapeFunctionVector, const ProcessInfo &rProcessInfo)
                { return rProperties.GetValue(rVariable, rGeometry, rShapeFunctionVector, rProcessInfo); })
            .def("GetValue", [](Properties &rProperties, const Variable<Array1d<double, 3>> &rVariable, const GeometryType &rGeometry, const Vector &rShapeFunctionVector, const ProcessInfo &rProcessInfo)
                { return rProperties.GetValue(rVariable, rGeometry, rShapeFunctionVector, rProcessInfo); })
            .def("GetValue", [](Properties &rProperties, const Variable<Array1d<double, 6>> &rVariable, const GeometryType &rGeometry, const Vector &rShapeFunctionVector, const ProcessInfo &rProcessInfo)
                { return rProperties.GetValue(rVariable, rGeometry, rShapeFunctionVector, rProcessInfo); })
            .def("GetValue", [](Properties &rProperties, const Variable<Array1d<double, 9>> &rVariable, const GeometryType &rGeometry, const Vector &rShapeFunctionVector, const ProcessInfo &rProcessInfo)
                { return rProperties.GetValue(rVariable, rGeometry, rShapeFunctionVector, rProcessInfo); })
            .def("GetValue", [](Properties &rProperties, const Variable<Array1d<double, 4>> &rVariable, const GeometryType &rGeometry, const Vector &rShapeFunctionVector, const ProcessInfo &rProcessInfo)
                { return rProperties.GetValue(rVariable, rGeometry, rShapeFunctionVector, rProcessInfo); })
